...
def train_net(net,
              ...
              n_class=1,
              img_size=512,
              checkpoint_path='/zyy/DS-TransUNet-master/checkpoints/FPN.pth'):  # 自定义保存文件的路径
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=30, gamma=0.1)  # 逐步减少学习率
    #scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, epochs // 5, lr / 10)
    criterion = criterion = WeightedDiceBCELoss()
    best_dice = 0
...
if __name__ == '__main__':
    ...
    net = smp.FPN(
        encoder_name="resnext50_32x4d",        # 选择解码器, 例如 mobilenet_v2 或 efficientnet-b7
