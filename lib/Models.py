...
class R2U_Net(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.Conv(d2)
        out = self.active(out)  # 添加sigmoid激活函数
        return out
...
class VNet(nn.Module):
    def __init__(self, in_channels, out_channels, n_base_filters=16):
        super(VNet, self).__init__()
        
        self.encoder = nn.ModuleList()
        self.decoder = nn.ModuleList()
        self.conv_final = nn.Conv3d(n_base_filters, out_channels, kernel_size=1)

        # Encoder
        for i in range(4):
            self.encoder.append(
                nn.Sequential(
                    nn.Conv3d(in_channels if i == 0 else n_base_filters * 2 ** i,
                              n_base_filters * 2 ** i,
                              kernel_size=3,
                              padding=1),
                    nn.BatchNorm3d(n_base_filters * 2 ** i),
                    nn.ReLU(inplace=True),
                    nn.Conv3d(n_base_filters * 2 ** i,
                              n_base_filters * 2 ** i,
                              kernel_size=3,
                              padding=1),
                    nn.BatchNorm3d(n_base_filters * 2 ** i),
        ...
        for i in range(3, -1, -1):
            self.decoder.append(
                nn.Sequential(
                    nn.ConvTranspose3d(n_base_filters * 2 ** (i + 1),
                                       n_base_filters * 2 ** i,
                                       kernel_size=2,
                                       stride=2),
                    nn.Conv3d(n_base_filters * 2 ** (i + 1),
                              n_base_filters * 2 ** i,
                              kernel_size=3,
                              padding=1),
                    nn.BatchNorm3d(n_base_filters * 2 ** i),
    ...
    def forward(self, x):
        ...
        for i, (decoder_layer, skip_connection) in enumerate(zip(self.decoder, decoder_inputs[:-1])):
            x = decoder_layer(x)
